...
def handle_client(client_socket, player_id): 
   ...
   clients.append(client_socket)
   client_socket.send(f"name:{suspects[player_id]}".encode())  # 플레이어 이름을 클라이언트에게 전송
   sleep(1) # 1초 대기
   client_socket.send(f"cards:{','.join(player_cards[player_id])}".encode())  # 분배된 카드 정보를 클라이언트에게 전송
   sleep(1) # 1초 대기
   client_socket.send(f"case_file:{','.join(case_file)}".encode())  # 사건 파일 정보를 클라이언트에게 전송
   while True:
      try:
